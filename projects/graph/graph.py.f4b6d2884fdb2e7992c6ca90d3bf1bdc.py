        ...
        while q.size() > 0:
            ...
            if vertex is not destination_vertex and vertex not in visited:
                ...
                for child in self.vertices[vertex]:
                    q.enqueue(child)
            elif vertex is destination_vertex:
                print(vertex)
                visited.append(vertex)
                break
        # look backwards through array (destination is always first)
        # initialize comparison variable as last item (our destination)
        # check previous item, is that a parent of comparison value?
